...
const DEFAULTS = {
  hero: {
    ...
    images: [
      ...
      { url: '/images/hero/hero-tracksuit.webp', key: null },
      { url: '/images/hero/hero-gingham.webp', key: null },
      { url: '/images/hero/hero-orangeset.webp', key: null },
    ], // jusqu'à 10 {url, key} — plusieurs photos défilent en fondu ; aucune = pictogramme "dress" par défaut
  },
  promoBanner: {
    tag: 'Offre limitée',
...
const { init, persist } = makePersistent({ r2Key: 'data/siteSettings.json', readLocal: readRaw, writeLocal: write });
...
async function update(patch) {
  ...
  if (Array.isArray(patch.hero?.images)) {
    next.hero.images = patch.hero.images.slice(0, 10);
  }
...
module.exports = { get, update, DEFAULTS, init };
