...
const { init, persist } = makePersistent({ r2Key: 'data/orders.json', readLocal: readAll, writeLocal: writeAll });
...
async function create({ customer, shipping, payment, items, subtotal, shippingCost, discount, total }) {
  const list = readAll();
