...
async function createDraftProduct(botToken, baseUrl, chatId, images, caption) {
  ...
  // Répartition volontaire : Claude (vision) sert uniquement à déduire la
  // catégorie à partir de la photo (c'est le seul des deux qui peut
  // vraiment "voir" l'image) ; DeepSeek (texte) reste seul responsable de
  // rédiger les descriptions. Chaque étape est best-effort — ne bloque
  // jamais la création du produit si non configuré ou en échec.
  if (needsCategory && claudeVision.isConfigured() && images.length) {
    try {
  ...
  if (deepseek.isConfigured()) {
    ...
    // Repli catégorie si Claude n'est pas configuré (ou a échoué).
    if (!categoryId) {
      try {
  ...
  if (!shortDescription && description) {
    ...
    shortDescriptionFromAI = descriptionFromAI;
  }
